...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        size_in, size_out = input_dim, hidden_dims[0] # first hidden_node
        for i in range(self.num_layers):
          self.params['W' + str(i + 1)] = weight_scale * np.random.randn(size_in, size_out)
          self.params['b' + str(i + 1)] = np.zeros(size_out)
          size_in = size_out
          if i < len(hidden_dims) - 1:
            size_out = hidden_dims[i + 1] # next hidden layer
          else:
            size_out = num_classes # last layer
