...
def affine_backward(dout, cache):
    ...
    db = np.dot(dout.T, np.ones(N)) # M * 1
...
def relu_backward(dout, cache):
    ...
    dx = np.array(dout, copy=True) # x>0
    dx[x <= 0] = 0
...
def softmax_loss(x, y):
    ...
    dx = probs.copy()
    dx[np.arange(N), y] -= 1 #-dSyi/dW + (log(sigma(eSj)))`
    dx /= N
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = (x, x_hat, gamma, batch_mean, batch_var, eps, bn_param)        
...
def batchnorm_backward(dout, cache):
    ...
    (x, x_hat, gamma, batch_mean, batch_var, eps, bn_param) = cache
    N, D = x.shape
    ...
    xmu = x - batch_mean
    dvar = np.sum(dx_hat * xmu, axis=0)
    ivar = 1 / (np.sqrt(batch_var + eps))
    dxmu1 = dx_hat * ivar
    dsqrtvar = -1. / ((np.sqrt(batch_var + eps))**2) * dvar
    dvar = 0.5 * 1. /np.sqrt(batch_var + eps) * dsqrtvar
...
def batchnorm_backward_alt(dout, cache):
    ...
    dvar = np.sum(dx_hat * (-0.5) * (x - batch_mean) * np.power(batch_var + eps, -1.5), axis = 0)

    # dmean = dx_hat * dx_hat/dmean + dvar * dvar/dmean
    # dx_hat/dmean = -1/var
    # dvar/dmean = -2/N * (x - mean)
    dmean = np.sum(dx_hat * -1 / np.sqrt(batch_var + eps), axis = 0) \
          + dvar * np.mean(-2 * (x - batch_mean), axis=0)
    ...
    dx = dx_hat * 1/np.sqrt(batch_var+eps) \
        + dvar * 2./N * (x - batch_mean) \
        + dmean / N
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    cache = (x, x_hat, gamma, layer_mean, layer_var, eps, ln_param)    
...
def layernorm_backward(dout, cache):
    ...
    (x, x_hat, gamma, layer_mean, layer_var, eps, ln_param) = cache
    N, D    = x.shape
    ...
    dvar = np.sum(dx_hat * (-0.5) * (x - layer_mean) * np.power(layer_var + eps, -1.5), axis = 0)

    dmean = np.sum(dx_hat * -1 / np.sqrt(layer_var + eps), axis = 0) \
          + dvar * np.mean(-2 * (x - layer_mean), axis=0)
    
    dx = dx_hat / np.sqrt(layer_var+eps) \
        + dvar * 2./N * (x - layer_mean) \
        + dmean / N
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    N, C, H, W = x.shape
    F, C, HH, WW = w.shape
    H_, W_ =  int((H +2 * pad - HH) / stride + 1), int((W + 2 * pad - WW) / stride + 1)
    out = np.zeros((N, F, H_, W_))

    # Perform a zero-pad operation on x to get the expanded matrix. 
    # Only the 3rd and 4th dimensions of x (that is, the dimension with pixel values) are expanded.
    # np.pad(array, pad_width, mode='constant', **kwargs)
    x_pad = np.pad(x, ((0, 0), (0, 0), (pad, pad), (pad, pad)), 'constant', constant_values=0)
    for n in range(N):
      out_x = x_pad[n] # (C, H+pad*2, W+pad*2)
      for f in range(F):
        kernel = w[f] # (C, HH, WW)
        for i in range(H_):
          for j in range(W_):
            conv_zone = out_x[:, i * stride: i * stride + HH, j * stride: j * stride + WW] # (C, HH, WW)
    ...
    for i in range(N):
      ...
      for j in range(F):
        ...
        for h_ in range(H_):
          for w_ in range(W_):
            db[j] += dout[i, j, h_, w_]
            dx_pad[i, :, h_ * stride:h_ * stride + HH, w_ * stride:w_ * stride + WW] += dout[i, j, h_, w_] * w_j
            dw[j] += dout[i, j, h_, w_] * x_i[:, h_ * stride:h_ * stride + HH, w_ * stride:w_ * stride + WW]
...
def max_pool_forward_naive(x, pool_param):
    ...
    H_, W_ = int(1 + (H - h) / stride), int(1 + (W - w) / stride)
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    N, C, H, W = x.shape
    # (N, C, H, W) -> (N, H, W, C) -> (N*H*W, C) for batch_normalization
    col_x = x.transpose(0,2,3,1).reshape(-1, C)
    col_out, cache = batchnorm_forward(col_x, gamma, beta, bn_param)
    out = col_out.reshape(N, H, W, C).transpose(0,3,1,2)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    N, C, H, W = dout.shape
    col_dout = dout.transpose(0,2,3,1).reshape(-1, C)
    dcol_x, dgamma, dbeta = batchnorm_backward_alt(col_dout, cache)
    dx = dcol_x.reshape(N,H,W,C).transpose(0,3,1,2)
